    ...
    this.container.addEventListener("pointerdown", (event) => {
      ...
      this.mouse.y =
        -(
          ...
          2 +
        1;
      let vector = new THREE.Vector3(this.mouse.x, this.mouse.y, 1).unproject(
        this.camera,
      );
    ...
    this.container.addEventListener("pointermove", (event) => {
      ...
      this.mouse1.y =
        -(
          ...
          2 +
        1;
      let vector = new THREE.Vector3(this.mouse1.x, this.mouse1.y, 1).unproject(
        this.camera,
      );
  ...
  setModelCenter(object, viewControl) {
    ...
    if (viewControl.autoCamera) {
      this.camera.position.x =
        centroid.x * (viewControl.centerX || 0) +
        width * (viewControl.width || 0);
      this.camera.position.y =
        centroid.y * (viewControl.centerY || 0) +
        height * (viewControl.height || 0);
      this.camera.position.z =
        centroid.z * (viewControl.centerZ || 0) +
        depth * (viewControl.depth || 0);
    } else {
